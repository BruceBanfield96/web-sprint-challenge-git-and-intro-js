...
function listOfNames(array) {
  const newNameArray = artists.slice ();
  for (let i = 0 ;i < array.length; i ++){
    newNameArray.push(array[i].name);
    
  } 
  newNameArray.splice (0,20);
  return newNameArray;
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  let updatedCopyArray = array.slice ();
  updatedCopyArray.push ({name, years, genre, nationality, bio });
  return updatedCopyArray;
}
...
function lotsOfArt(array) {
  ...
  for (let i = 0; i < array.length; i++){
    if (array[i].paintings > 100){
      experiencedPainters.push(array[i].name);
    } 
  }
  return experiencedPainters;
}
